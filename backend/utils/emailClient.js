...
async function sendEmailVerificationOtp({ to, otp }) {
  if (!emailConfigReady()) {
    // Development fallback: keep the verification flow testable without SMTP.
    // Never log OTPs in production. For Gmail, use an App Password with:
    // EMAIL_HOST=smtp.gmail.com, EMAIL_PORT=465, EMAIL_SECURE=true.
    if (process.env.NODE_ENV !== 'production') {
  ...
  await transporter.sendMail({
    ...
    text: `Your Grand Egyptian Museum Tourist Guide verification number is ${otp}. It expires in ${OTP_TTL_MINUTES} minutes.`,
    html: `
      <p>Your Grand Egyptian Museum Tourist Guide verification number is:</p>
      <h2 style="letter-spacing: 4px;">${otp}</h2>
